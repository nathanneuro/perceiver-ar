...
def get_config(arg_string):
            ...
            config=dict(
                ...
                model=dict(
                    perceiver_ar_kwargs=dict(
                        ...
                        max_wavelength=config.get_oneway_ref("max_context_length"),
                ...
                data=dict(
                    ...
                    max_context_length=config.get_oneway_ref("max_context_length"),
    ...
    config.update_from_flattened_dict(
        {k: v for k, v in get_sweep(sweep, train_device_count=1, eval_device_count=1)}
    )
...
def _get_batch_sweep(
    train_batch_sizes,
    train_batch_sizes_per_device,
    train_device_count,
...
def get_sweep(sweep_name, train_device_count, eval_device_count):
    ...
    if sweep_name == "dummy_use_positions_from_data":
        ...
        train_batch_sizes_per_device = [1]
        eval_batch_size_per_device = 4
    elif sweep_name == "mnist":
        sweep_parameters = [
            # ----- Start active sweep ------
            ("max_context_length", 1024),
            (f"{MODEL_KWARGS}.max_wavelength", 1024),
            (f"{MODEL_KWARGS}.input_embed_dim", 1024),
            (f"{MODEL_KWARGS}.num_cross_attend_heads", 4),
            (f"{MODEL_KWARGS}.fraction_to_rotate", 0.5),
            (f"{MODEL_KWARGS}.num_transformers_per_block", 2),
            (f"{MODEL_KWARGS}.z_index_dim", 1024),
            ...
            (f"{EXP_CONFIG}.optimizer.schedule_type", "constant"),
            # ----- End active sweep ------
            (f"{MODEL_KWARGS}.num_transformer_heads", 4),
            (f"{MODEL_KWARGS}.cross_attend_widening_factor", 4),
            (f"{MODEL_KWARGS}.transformer_widening_factor", 4),
            (f"{MODEL_KWARGS}.activation_name", "sq_relu"),
            (f"{EXP_CONFIG}.optimizer.max_norm", 1.0),
        ...
        sweep_parameters = [
            ...
            (f"{EXP_CONFIG}.optimizer.schedule_type", "constant"),
            # ----- End active sweep ------
            (f"{MODEL_KWARGS}.num_transformer_heads", 12),
            (f"{MODEL_KWARGS}.cross_attend_widening_factor", 4),
            (f"{MODEL_KWARGS}.transformer_widening_factor", 4),
            (f"{MODEL_KWARGS}.activation_name", "sq_relu"),
            (f"{EXP_CONFIG}.optimizer.max_norm", 1.0),
            (f"{EXP_CONFIG}.optimizer.optimizer", "adam"),
            (f"{EXP_CONFIG}.optimizer.base_lr", 3e-4),
            (f"{MODEL_KWARGS}.num_z_channels", 768),
            (f"{EXP_CONFIG}.optimizer.warmup_steps", 10),  # 1e3
            (f"{MODEL_KWARGS}.dropout_prob", 0.1),
            (f"{MODEL_KWARGS}.latent_dropout_prob", 0.0),
            ("random_seed", 1),
        ]
...
class Experiment(experiment.AbstractExperiment):
    ...
    def _get_lr_schedule(self):
        ...
        warmup_schedule = optax.linear_schedule(
            init_value=init_lr, end_value=base_lr, transition_steps=warmup_steps
        )
        ...
        schedule_fn = optax.join_schedules(
            (warmup_schedule, schedule_fn), boundaries=[warmup_steps]
        )
    ...
    def _initialize_train(self):
        ...
        if self.config.optimizer.optimizer == "adafactor":
            ...
            if self.config.optimizer.optimizer == "sgd":
                optax_chain.extend([optax.trace(**self.config.optimizer.sgd_kwargs)])
        ...
        if self._params is None:
            ...
            def init_net(init_rng, inputs):
                ...
                return self.forward.init(
                    init_rng, input_events, input_idxs, context_events, is_training=True
                )
    ...
    def _build_train_input(self) -> tf.data.Dataset:
        ...
        return self._load_data(
            ...
            batch_dims=[
                ...
                self.config.training.num_microbatches,
    ...
    def _loss_fn(
        ...
        def add_aux_loss_metrics(start_idx, end_idx, name):
            ...
            loss_scalars.update(
                {
                    f"{name}_events_loss": events_loss,
                    f"num_{name}_events": num_events,
                    f"{name}_events_loss_per_event": jnp.where(
                        num_events > 0, events_loss / num_events, 0
                    ),
                    f"{name}_events_z_loss": events_z_loss,
                    f"{name}_events_z_loss_per_event": jnp.where(
                        num_events > 0, events_z_loss / num_events, 0
                    ),
                }
            )
    ...
    def _update_func(
        ...
        def _microbatch_step(loop_cnt, loop_state):
            ...
            grads, (loss_scalars, state) = grad_loss_fn(
                params, state, microbatch_input, grad_rng[loop_cnt]
            )
            grads_accum = jax.tree_util.tree_map(jnp.add, grads_accum, grads)
            loss_scalars_accum = jax.tree_util.tree_map(
                jnp.add, loss_scalars_accum, loss_scalars
            )
            return grads_accum, loss_scalars_accum, state

        grads_accum_init = jax.tree_map(lambda x: jnp.zeros(x.shape, x.dtype), params)
        ...
        scalars = {
            ...
            "train_max_context": max_context,
            "n_params (M)": float(n_params / 1e6),
            "train_num_examples": num_examples,
            "global_gradient_norm": optax.global_norm(grads_accum),
            "global_gradient_norm_per_example": optax.global_norm(grads_accum)
            / num_examples,
        }
    ...
    EVAL_INDEX_BUCKETS = np.stack([EVAL_INDEX_BUCKETS[:-1], EVAL_INDEX_BUCKETS[1:]]).T
    ...
    def _eval_batch(
        ...
        for bucket in Experiment.EVAL_INDEX_BUCKETS:
            ...
            in_seq_bucket = jnp.logical_and(
                target_token_idx >= bucket[0], target_token_idx < bucket[1]
            )

            extra_scalars[
                f"eval_sequence_{bucket[0]}_{bucket[1]}_num_examples"
            ] = in_seq_bucket
            extra_scalars[f"eval_sequence_{bucket[0]}_{bucket[1]}_loss"] = (
                in_seq_bucket * loss
            )
        batch_scalars.update(extra_scalars)

        # Don't include scores for padding events.
        batch_scalars = jax.tree_map(
            lambda x: x * (targets != dataset.PAD_ID), batch_scalars
        )
    ...
    def _eval_epoch(self, rng, global_step):
        ...
        extra_scalars = {}
        for bucket in Experiment.EVAL_INDEX_BUCKETS:
            extra_scalars[f"eval_sequence_{bucket[0]}_{bucket[1]}_num_examples"] = 0.0
        ...
        def calculate_metrics_from_scalars(scalars, n_batches):
            ...
            for bucket in Experiment.EVAL_INDEX_BUCKETS:
                bucket_loss_k = f"eval_sequence_{bucket[0]}_{bucket[1]}_loss"
                bucket_num_examples_k = (
                    f"eval_sequence_{bucket[0]}_{bucket[1]}_num_examples"
                )
                metrics[bucket_loss_k] = jnp.where(
                    metrics[bucket_num_examples_k] > 0,
                    metrics[bucket_loss_k] / metrics[bucket_num_examples_k],
                    0.0,
                )
...
def restore_state_to_in_memory_checkpointer(restore_path, config):
    ...
    jaxline_state = dict(
        global_step=pretrained_state["global_step"], experiment_module=dummy_experiment
    )
